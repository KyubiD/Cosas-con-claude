...
local function buildTemplate(rng, role, botName)
	for attempt = 1, 3 do
		...
		if ok and model then
			...
			if look and Tac.dressModel then
				local dressed, err = pcall(Tac.dressModel, model, look, rng, botName)
...
--  uniforme es el color del cuerpo, y el equipo (casco, chaleco, mochila,
--  botas...) son piezas soldadas al cuerpo dentro de la carpeta
--  "BotEquipo". Sin colision ni raycasts: no frenan balas ni cambian la
--  hitbox. Los estilos viven en BotConfig.Avatar.Styles.
--  (26/09) Cada pieza con su material y el detalle (camuflaje, costuras,
--  bolsillos, nombre, MOLLE...) dibujado con SurfaceGui "BotDetalle"
--  (atributo Aspecto: CorpseServer lo deja en el cuerpo).
-- --------------------------------------------------------------------------
...
function Tac.styleDescription(desc, A, style, rng, role)
	...
	if style.Kind == "Civil" and A.Shirts and #A.Shirts > 0 and rng:NextNumber() < (style.CatalogChance or 0) then
		...
		if style.Kind == "Civil" and not look.shortSleeves then
			--  Manga larga de civil: buzo con capucha, campera abierta o remera.
			local roll = rng:NextNumber()
			if roll < (style.HoodieChance or 0.3) then
				look.hoodie = true
			elseif roll < (style.HoodieChance or 0.3) + (style.JacketChance or 0.25) then
				look.jacket = true
				local inner = { Color3.fromRGB(236, 236, 232), Color3.fromRGB(40, 40, 44), randomFrom(style.Top, rng) }
				look.innerColor = inner[rng:NextInteger(1, #inner)]
			end
		end
	end
...
--  [aspecto 26/09] Materiales segun la pieza (tela solo en lo que es tela) y
--  detalle dibujado con SurfaceGui (camuflaje, costuras, bolsillos, cinta con
--  el nombre, parches, MOLLE, cordones...): sin imagenes ni IDs.
function Tac.dressModel(model, look, rng, botName)
	local style = look.style
	local A = Config.Avatar
	local level = A.DetailLevel or 2
	local folder = Instance.new("Folder")
	folder.Name = "BotEquipo"
	local count, maxParts = 0, A.MaxGearParts or 40
	local M = Enum.Material
	local dark = Color3.fromRGB(28, 28, 30)
	local metal = Color3.fromRGB(120, 122, 126)
	local pieces = {}		-- [nombre] = pieza (para pintarla despues)
	...
	local function add(attach, name, size, offset, color, material, shape)
		...
		piece.Size = size
		piece.Color = color
		piece.Material = material or M.SmoothPlastic
		...
		weld.Parent = piece
		piece.Parent = folder
		pieces[name] = pieces[name] or piece
		return piece
	end

	--  Cuerpo: liso (la textura la pone el dibujo), nada de "tejido".
	if not look.catalog then
		for _, name in ipairs(Tac.CLOTH_PARTS) do
			local part = body(name)
			if part then part.Material = M.SmoothPlastic end
		end
	end
	...
	if head then
		...
		if hg == "Casco" then
			add(head, "Casco", Vector3.new(s.X * 1.26, s.Y * 0.84, s.Z * 1.3), CFrame.new(0, s.Y * 0.3, s.Z * 0.03), hc, M.SmoothPlastic, "Ball")
			add(head, "CascoBorde", Vector3.new(0.12, s.X * 1.34, s.Z * 1.34), CFrame.new(0, s.Y * 0.27, s.Z * 0.04) * CFrame.Angles(0, 0, math.rad(90)),
				Tac.tone(hc, 0.85), M.SmoothPlastic, "Cylinder")
			--  Rieles a los costados y parche de velcro atras.
			for _, side in ipairs({ -1, 1 }) do
				add(head, "Riel", Vector3.new(0.07, 0.14, s.Z * 0.75), CFrame.new(side * s.X * 0.64, s.Y * 0.36, s.Z * 0.04), Tac.tone(hc, 0.6), M.SmoothPlastic)
			end
			add(head, "Velcro", Vector3.new(s.X * 0.4, s.Y * 0.22, 0.06), CFrame.new(0, s.Y * 0.42, s.Z * 0.66), Tac.tone(hc, 0.75), M.Fabric)
			if look.nvg then
				add(head, "SoporteVN", Vector3.new(0.35, 0.3, 0.2), CFrame.new(0, s.Y * 0.5, -s.Z * 0.62), dark, M.Metal)
			end
			if look.glasses then
				add(head, "GafasCasco", Vector3.new(s.X * 0.85, s.Y * 0.17, 0.16), CFrame.new(0, s.Y * 0.42, -s.Z * 0.58), dark, M.SmoothPlastic)
				add(head, "Lente", Vector3.new(s.X * 0.7, s.Y * 0.11, 0.05), CFrame.new(0, s.Y * 0.42, -s.Z * 0.58 - 0.08), Color3.fromRGB(70, 90, 110), M.Glass)
				look.glasses = false
			end
		elseif hg == "Gorra" then
			add(head, "Gorra", Vector3.new(s.X * 1.08, s.Y * 0.56, s.Z * 1.1), CFrame.new(0, s.Y * 0.32, 0), hc, M.SmoothPlastic, "Ball")
			local visor = rng:NextNumber() < 0.2 and 1 or -1		-- a veces para atras
			add(head, "Visera", Vector3.new(s.X * 0.72, 0.07, s.Z * 0.5), CFrame.new(0, s.Y * 0.2, visor * s.Z * 0.66), Tac.tone(hc, 0.85), M.SmoothPlastic)
			add(head, "Boton", Vector3.new(0.14, 0.08, 0.14), CFrame.new(0, s.Y * 0.6, 0), Tac.tone(hc, 0.8), M.SmoothPlastic)
		elseif hg == "Gorro" then
			add(head, "Gorro", Vector3.new(s.X * 1.1, s.Y * 0.8, s.Z * 1.12), CFrame.new(0, s.Y * 0.3, 0), hc, M.Fabric, "Ball")
			add(head, "Doblez", Vector3.new(0.2, s.X * 1.14, s.Z * 1.16), CFrame.new(0, s.Y * 0.08, 0) * CFrame.Angles(0, 0, math.rad(90)), Tac.tone(hc, 0.9), M.Fabric, "Cylinder")
			if rng:NextNumber() < 0.3 then
				add(head, "Pompon", Vector3.new(0.4, 0.4, 0.4), CFrame.new(0, s.Y * 0.72, 0), Tac.jitter(hc, rng, 0.2), M.Fabric, "Ball")
			end
		elseif hg == "Boonie" then
			add(head, "Boonie", Vector3.new(s.X * 1.08, s.Y * 0.56, s.Z * 1.1), CFrame.new(0, s.Y * 0.33, 0), hc, M.Fabric, "Ball")
			add(head, "Ala", Vector3.new(0.06, s.X * 1.9, s.X * 1.9), CFrame.new(0, s.Y * 0.16, 0) * CFrame.Angles(0, 0, math.rad(90)), hc, M.Fabric, "Cylinder")
			add(head, "Cinta", Vector3.new(0.1, s.X * 1.12, s.Z * 1.14), CFrame.new(0, s.Y * 0.2, 0) * CFrame.Angles(0, 0, math.rad(90)), Tac.tone(hc, 0.75), M.Fabric, "Cylinder")
		elseif hg == "Boina" then
			add(head, "Boina", Vector3.new(s.X * 1.15, s.Y * 0.34, s.Z * 1.15), CFrame.new(s.X * 0.08, s.Y * 0.47, 0) * CFrame.Angles(0, 0, math.rad(-10)), hc, M.Fabric, "Ball")
			add(head, "Insignia", Vector3.new(0.16, 0.2, 0.05), CFrame.new(-s.X * 0.25, s.Y * 0.5, -s.Z * 0.55), Color3.fromRGB(200, 170, 70), M.Metal)
		end
		if look.headset then
			for _, side in ipairs({ -1, 1 }) do
				add(head, "Auricular", Vector3.new(0.2, s.Y * 0.45, s.Y * 0.45), CFrame.new(side * (s.X * 0.5 + 0.1), s.Y * 0.02, 0), dark, M.SmoothPlastic, "Cylinder")
			end
			if not hg then
				add(head, "Diadema", Vector3.new(s.X * 1.12, 0.1, 0.2), CFrame.new(0, s.Y * 0.56, 0), dark, M.SmoothPlastic)
			end
			if style.Kind == "Militar" then
				add(head, "Microfono", Vector3.new(0.06, 0.06, s.Z * 0.45), CFrame.new(s.X * 0.52, -s.Y * 0.12, -s.Z * 0.3), dark, M.SmoothPlastic)
			end
		end
		if look.glasses then
			add(head, "Gafas", Vector3.new(s.X * 0.82, s.Y * 0.16, 0.1), CFrame.new(0, s.Y * 0.08, -s.Z * 0.5 - 0.03), dark, M.SmoothPlastic)
		end
	end

	--  Torso: chaleco porta placas, mochila, bufanda, capucha.
	local torso = body("UpperTorso")
	if torso then
		local s = torso.Size
		local gc = look.gearColor
		if look.vest then
			add(torso, "Chaleco", Vector3.new(s.X * 1.08, s.Y * 0.72, s.Z * 1.35), CFrame.new(0, -s.Y * 0.04, 0), gc, M.SmoothPlastic)
			--  Tirantes sobre los hombros.
			for _, side in ipairs({ -1, 1 }) do
				add(torso, "Tirante", Vector3.new(s.X * 0.2, s.Y * 0.22, s.Z * 1.3), CFrame.new(side * s.X * 0.3, s.Y * 0.4, 0), Tac.tone(gc, 0.9), M.SmoothPlastic)
			end
			for i, x in ipairs({ -0.3, 0, 0.3 }) do
				add(torso, "Bolsillo" .. i, Vector3.new(s.X * 0.24, s.Y * 0.24, s.Z * 0.22), CFrame.new(x * s.X, -s.Y * 0.22, -s.Z * 0.78), Tac.jitter(gc, rng, 0.08), M.SmoothPlastic)
			end
			if rng:NextNumber() < 0.5 then
				add(torso, "Radio", Vector3.new(0.22, 0.4, 0.2), CFrame.new(-s.X * 0.32, s.Y * 0.18, -s.Z * 0.78), dark, M.SmoothPlastic)
				add(torso, "Antena", Vector3.new(0.04, 0.5, 0.04), CFrame.new(-s.X * 0.36, s.Y * 0.55, -s.Z * 0.78), dark, M.Metal)
			end
		end
		if look.backpack then
			local bag = Tac.jitter(gc, rng, 0.1)
			add(torso, "Mochila", Vector3.new(s.X * 0.72, s.Y * 0.85, s.Z * 0.6), CFrame.new(0, -s.Y * 0.02, s.Z * 0.95), bag, M.SmoothPlastic)
			if not look.vest then
				for _, side in ipairs({ -1, 1 }) do
					add(torso, "Correa", Vector3.new(s.X * 0.12, s.Y * 0.9, 0.05), CFrame.new(side * s.X * 0.26, 0, -s.Z * 0.52), Tac.tone(bag, 0.75), M.SmoothPlastic)
				end
			end
			if style.Kind == "Militar" and rng:NextNumber() < 0.5 then
				add(torso, "Saco", Vector3.new(s.X * 0.7, s.Z * 0.42, s.Z * 0.42), CFrame.new(0, s.Y * 0.52, s.Z * 0.95), Tac.jitter(bag, rng, 0.15), M.Fabric, "Cylinder")
			end
		end
		if look.scarf then
			add(torso, "Bufanda", Vector3.new(s.X * 0.56, s.Y * 0.2, s.Z * 1.12), CFrame.new(0, s.Y * 0.5, 0), look.scarf, M.Fabric)
		end
		if look.hoodie then
			add(torso, "Capucha", Vector3.new(s.X * 0.6, s.Y * 0.3, s.Z * 0.35), CFrame.new(0, s.Y * 0.42, s.Z * 0.62), Tac.tone(look.top, 0.92), M.SmoothPlastic)
		end
	end
	local lower = body("LowerTorso")
	if lower and look.belt then
		local s = lower.Size
		local beltColor = Tac.jitter(style.Kind == "Militar" and Tac.tone(look.gearColor, 0.8) or dark, rng, 0.1)
		add(lower, "Cinturon", Vector3.new(s.X * 1.06, s.Y * 0.9, s.Z * 1.12), CFrame.new(), beltColor, M.Leather)
		add(lower, "Hebilla", Vector3.new(0.3, s.Y * 0.6, 0.06), CFrame.new(0, 0, -s.Z * 0.58), metal, M.Metal)
	end

	--  Brazos y piernas: mangas, guantes / manos, rodilleras, botas con suela.
	for _, side in ipairs({ "Left", "Right" }) do
		...
		if look.shortSleeves and upperArm then
			...
			add(upperArm, side .. "Manga", Vector3.new(s.X * 1.07, s.Y * 0.5, s.Z * 1.07), CFrame.new(0, s.Y * 0.26, 0), look.top, M.SmoothPlastic)
		end
		if hand then
			local s = hand.Size
			if look.gloves then
				add(hand, side .. "Guante", s * 1.1, CFrame.new(), look.gloves, M.Leather)
				add(hand, "Nudillos", Vector3.new(s.X * 0.8, s.Y * 0.5, 0.05), CFrame.new(0, s.Y * 0.1, -s.Z * 0.58), Tac.tone(look.gloves, 0.7), M.Rubber)
			elseif not look.shortSleeves and not look.catalog then
				add(hand, "Mano", s * 1.06, CFrame.new(), look.skin, M.Plastic)
			end
		end
		if lowerLeg then
			local s = lowerLeg.Size
			if look.kneePads then
				add(lowerLeg, side .. "Rodillera", Vector3.new(s.X * 0.74, s.Y * 0.3, s.Z * 0.32), CFrame.new(0, s.Y * 0.3, -s.Z * 0.5), Tac.jitter(look.gearColor, rng, 0.1), M.Rubber)
			end
			if look.boots and style.Kind == "Militar" then
				add(lowerLeg, side .. "Cana", Vector3.new(s.X * 1.08, s.Y * 0.34, s.Z * 1.08), CFrame.new(0, -s.Y * 0.33, 0), look.boots, M.Leather)
			end
		end
		if foot and look.boots then
			local s = foot.Size
			local military = style.Kind == "Militar"
			add(foot, side .. "Bota", Vector3.new(s.X * 1.1, s.Y * 1.35, s.Z * 1.16), CFrame.new(0, s.Y * 0.1, -s.Z * 0.03), look.boots, military and M.Leather or M.SmoothPlastic)
			--  Suela: goma oscura (militar) o blanca (zapatilla).
			local sole = military and Color3.fromRGB(24, 22, 20) or (rng:NextNumber() < 0.6 and Color3.fromRGB(235, 235, 230) or dark)
			add(foot, "Suela", Vector3.new(s.X * 1.14, 0.14, s.Z * 1.2), CFrame.new(0, -s.Y * 0.55, -s.Z * 0.03), sole, M.Rubber)
		end
	end

	if level <= 0 then
		--  Sin dibujo: el camuflaje viejo con piezas.
		Tac.camoPieces(model, look, rng, add)
	end
	folder.Parent = model
	model:SetAttribute("BotEstilo", style.Name)
	if level > 0 then
		--  Al final: si el dibujo fallara, el equipo ya esta puesto.
		Tac.paintModel(model, look, rng, botName, pieces, level)
	end
end

--  Camuflaje con piezas (DetailLevel = 0).
function Tac.camoPieces(model, look, rng, add)
	local camo = look.camo
	local patches = look.style.CamoPatches or Config.Avatar.CamoPatches or 6
	if type(camo) ~= "table" or #camo == 0 or patches <= 0 then return end
	local spots = {
		...
		{ "LeftLowerLeg", "front" }, { "RightLowerLeg", "front" },
	}
	for i = #spots, 2, -1 do
	...
	for i = 1, math.min(patches, #spots) do
		...
		if part and not (name == "UpperTorso" and face == "front" and look.vest) then
			local s = part.Size
			local width, height = s.X * rng:NextNumber(0.3, 0.55), s.Y * rng:NextNumber(0.22, 0.42)
			local sign = face == "front" and -1 or 1
			add(part, "Camuflaje", Vector3.new(width, height, 0.04),
				CFrame.new(rng:NextNumber(-1, 1) * (s.X - width) * 0.5, rng:NextNumber(-1, 1) * (s.Y - height) * 0.5, sign * (s.Z * 0.5 + 0.02)),
				Tac.jitter(randomFrom(camo, rng), rng, 0.05), Enum.Material.SmoothPlastic)
		end
	end
end

-- --------------------------------------------------------------------------
--  Dibujo con SurfaceGui: marcos de UI pegados a las caras de las piezas.
--  Coordenadas en escala 0-1 de la cara. MaxDistance: de lejos no se dibuja
--  (rendimiento). No tienen colision ni afectan nada del juego.
-- --------------------------------------------------------------------------
function Tac.tone(color, k)
	return Color3.new(math.clamp(color.R * k, 0, 1), math.clamp(color.G * k, 0, 1), math.clamp(color.B * k, 0, 1))
end

function Tac.rect(parent, x, y, w, h, color, z, opts)
	...
	frame.ZIndex = z or 1
	if opts then
		if opts.alpha then frame.BackgroundTransparency = opts.alpha end
		if opts.round then
			local corner = Instance.new("UICorner")
			corner.CornerRadius = UDim.new(opts.round, 0)
			corner.Parent = frame
		end
		if opts.stroke then
			local stroke = Instance.new("UIStroke")
			stroke.Color = opts.stroke
			stroke.Thickness = opts.strokeWidth or 1.5
			stroke.Transparency = opts.strokeAlpha or 0.25
...
function Tac.text(parent, x, y, w, h, text, color, z)
...
--  Una cara pintada (el fondo es la pieza misma) con costura en el borde si
--  seam. Devuelve el lienzo (en escala) y el tamano de la cara en studs.
function Tac.canvas(part, faceName, color, seam)
	local s = part.Size
	local gui = Instance.new("SurfaceGui")
	gui.Name = "BotDetalle"
	gui.Face = Enum.NormalId[faceName]
	gui.SizingMode = Enum.SurfaceGuiSizingMode.PixelsPerStud
	gui.PixelsPerStud = 50
	gui.LightInfluence = 1
	gui.MaxDistance = Config.Avatar.DetailDistance or 110
	gui.ClipsDescendants = true
	gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	gui:SetAttribute("Aspecto", true)
	local width = (faceName == "Left" or faceName == "Right") and s.Z or s.X
	local height = (faceName == "Top" or faceName == "Bottom") and s.Z or s.Y
	if seam then
		--  Costura: un marco apenas hacia adentro con borde oscuro.
		Tac.rect(gui, 0.03, 0.02, 0.94, 0.96, color, 8, { alpha = 1, stroke = Tac.tone(color, 0.55), strokeWidth = 1.2, strokeAlpha = 0.55 })
	end
	gui.Parent = part
	return gui, width, height
end

--  Camuflaje dibujado. "Manchas" (bosque / desierto / nieve) o "Digital".
function Tac.paintCamo(base, width, height, pattern, palette, rng, amount)
	if type(palette) ~= "table" or #palette == 0 or not pattern or pattern == "Liso" then return end
	local area = width * height
	amount = math.max(2, math.floor(amount * math.clamp(area / 3.2, 0.35, 1.2) + 0.5))
	if pattern == "Digital" then
		local cellW, cellH = 0.18 / width, 0.18 / height		-- pixeles de 0.18 studs
		for _ = 1, amount * 2 do
			local color = randomFrom(palette, rng)
			local horizontal = rng:NextNumber() < 0.55
			local length = rng:NextInteger(1, 4)
			local w = math.min(horizontal and cellW * length or cellW * rng:NextInteger(1, 2), 1)
			local h = math.min(horizontal and cellH * rng:NextInteger(1, 2) or cellH * length, 1)
			local x = math.floor(rng:NextNumber(0, math.max(1 - w, 0)) / cellW) * cellW
			local y = math.floor(rng:NextNumber(0, math.max(1 - h, 0)) / cellH) * cellH
			Tac.rect(base, x, y, w, h, color, 2)
		end
		return
	end
	for _ = 1, amount do
		local color = randomFrom(palette, rng)
		--  Una mancha son 2-3 ovalos que se pisan: forma irregular.
		local bw, bh = rng:NextNumber(0.35, 0.85) / width, rng:NextNumber(0.2, 0.5) / height
		bw, bh = math.min(bw, 0.8), math.min(bh, 0.7)
		local x, y = rng:NextNumber(0, 1 - bw), rng:NextNumber(0, 1 - bh)
		Tac.rect(base, x, y, bw, bh, color, 2, { round = 0.5 })
		if rng:NextNumber() < 0.55 then
			local sw, sh = bw * rng:NextNumber(0.5, 0.8), bh * rng:NextNumber(0.6, 1)
			local sx = math.clamp(x + rng:NextNumber(-0.5, 0.8) * bw, 0, 1 - sw)
			local sy = math.clamp(y + rng:NextNumber(-0.5, 0.6) * bh, 0, 1 - sh)
			Tac.rect(base, sx, sy, sw, sh, color, 2, { round = 0.5 })
		end
	end
	if pattern == "Pintas" then
		--  Desierto: pintitas oscuras encima.
		local chip = palette[#palette]
		for _ = 1, math.ceil(amount * 0.8) do
			local d = rng:NextNumber(0.08, 0.16)
			local w, h = d / width, d / height
			Tac.rect(base, rng:NextNumber(0, 1 - w), rng:NextNumber(0, 1 - h), w, h, chip, 3, { round = 0.5 })
		end
	end
end

--  Bolsillo con solapa y boton.
function Tac.pocket(base, x, y, w, h, color, z)
	local pocket = Tac.rect(base, x, y, w, h, Tac.tone(color, 0.93), z or 4, { stroke = Tac.tone(color, 0.55), strokeWidth = 1.2 })
	Tac.rect(pocket, 0, 0, 1, 0.32, Tac.tone(color, 0.82), (z or 4) + 1, { stroke = Tac.tone(color, 0.5), strokeWidth = 1 })
	Tac.rect(pocket, 0.42, 0.18, 0.16, 0.14, Tac.tone(color, 0.6), (z or 4) + 2, { round = 0.5 })
	return pocket
end

--  Parche de hombro: bandera de tres franjas, escudo redondo o rango.
function Tac.patch(base, rng, dark)
	local kind = rng:NextInteger(1, 3)
	if kind == 1 then
		local palettes = {
			{ Color3.fromRGB(0, 104, 71), Color3.fromRGB(240, 240, 240), Color3.fromRGB(206, 17, 38) },
			{ Color3.fromRGB(0, 56, 147), Color3.fromRGB(240, 240, 240), Color3.fromRGB(206, 17, 38) },
			{ Color3.fromRGB(252, 209, 22), Color3.fromRGB(0, 56, 147), Color3.fromRGB(206, 17, 38) },
			{ Color3.fromRGB(60, 60, 60), Color3.fromRGB(90, 90, 90), Color3.fromRGB(60, 60, 60) },		-- apagada
		}
		local colors = palettes[rng:NextInteger(1, #palettes)]
		local flag = Tac.rect(base, 0.2, 0.12, 0.6, 0.3, colors[1], 5, { stroke = dark, strokeWidth = 1 })
		Tac.rect(flag, 0.333, 0, 0.334, 1, colors[2], 6)
		Tac.rect(flag, 0.667, 0, 0.333, 1, colors[3], 6)
	elseif kind == 2 then
		local outer = Tac.rect(base, 0.22, 0.1, 0.56, 0.36, Tac.tone(dark, 1.6), 5, { round = 0.5, stroke = dark })
		Tac.rect(outer, 0.25, 0.25, 0.5, 0.5, Color3.fromRGB(190, 160, 70), 6, { round = 0.5 })
	else
		for i = 0, rng:NextInteger(1, 3) - 1 do
			Tac.rect(base, 0.28, 0.12 + i * 0.1, 0.44, 0.06, Color3.fromRGB(190, 170, 110), 5)
		end
	end
end

--  Nombre para la cinta del pecho: la primera palabra en mayusculas (sin
--  el "xX...Xx").
function Tac.tapeName(name)
	local word = string.match(name or "", "[^%s_%d%p]+") or "BOT"
	word = string.gsub(word, "^xX", "")
	word = string.gsub(word, "Xx$", "")
	if word == "" then word = "BOT" end
	local cut = utf8.len(word) and utf8.len(word) > 9 and utf8.offset(word, 10)
	if cut then word = string.sub(word, 1, cut - 1) end
	return string.upper(word)
end

function Tac.paintModel(model, look, rng, botName, pieces, level)
	local style = look.style
	local military = style.Kind == "Militar"
	local pattern = style.Pattern or (military and "Manchas" or "Liso")
	local camo = look.camo
	local amount = style.CamoAmount or 6
	local dark = Color3.fromRGB(28, 28, 30)
	local function body(name) return model:FindFirstChild(name) end
	local function paint(part, faceName, color, withCamo, seam)
		if not part then return nil end
		local base, w, h = Tac.canvas(part, faceName, color, seam)
		if withCamo then Tac.paintCamo(base, w, h, pattern, camo, rng, amount) end
		return base, w, h
	end
	local top, bottom = look.top, look.bottom
	...
	if not look.catalog then
		...
		if torso then
			if not look.vest then
				local front = paint(torso, "Front", top, military, true)
				if front then
					if military then
						Tac.rect(front, 0.485, 0.04, 0.03, 0.92, Tac.tone(top, 0.7), 3)		-- cierre
						Tac.pocket(front, 0.1, 0.24, 0.3, 0.3, top)
						Tac.pocket(front, 0.6, 0.24, 0.3, 0.3, top)
						local tape = Tac.rect(front, 0.07, 0.08, 0.36, 0.12, Tac.tone(top, 0.75), 6)
						Tac.text(tape, 0.05, 0.05, 0.9, 0.9, Tac.tapeName(botName), Color3.fromRGB(20, 20, 20), 7)
					else
						Tac.paintShirt(front, look, rng)
					end
				end
			end
			paint(torso, "Back", top, military, true)
		end
		--  Brazos
		for _, side in ipairs({ "Left", "Right" }) do
			local upperArm, lowerArm = body(side .. "UpperArm"), body(side .. "LowerArm")
			local armColor = look.shortSleeves and nil or top
			if armColor then
				local outer = paint(upperArm, side, armColor, military)
				if outer and military and (level >= 2 or side == "Left") and rng:NextNumber() < 0.7 then
					Tac.patch(outer, rng, dark)
				end
				if level >= 2 then
					paint(upperArm, "Front", armColor, military)
					paint(upperArm, "Back", armColor, military)
					local cuff = paint(lowerArm, side, armColor, military)
					if cuff then Tac.rect(cuff, 0, 0.82, 1, 0.1, Tac.tone(armColor, 0.75), 4) end
				end
			end
		end
		--  Piernas
		for _, side in ipairs({ "Left", "Right" }) do
			local upperLeg, lowerLeg = body(side .. "UpperLeg"), body(side .. "LowerLeg")
			local front = paint(upperLeg, "Front", bottom, military, true)
			local back = paint(upperLeg, "Back", bottom, military, true)
			if military then
				local outer = paint(upperLeg, side, bottom, true)
				if outer then Tac.pocket(outer, 0.2, 0.35, 0.6, 0.45, bottom) end
			elseif front then
				--  Jeans / pantalon: bolsillo (del lado de la cadera: en la cara de
				--  adelante la x crece hacia la izquierda del personaje), costura y
				--  rodilla gastada.
				Tac.rect(front, side == "Left" and 0.55 or 0.05, 0.02, 0.4, 0.22, Tac.tone(bottom, 0.8), 3, { round = 0.5, alpha = 0.3 })
				Tac.rect(front, side == "Left" and 0.08 or 0.88, 0, 0.04, 1, Tac.tone(bottom, 0.75), 3, { alpha = 0.2 })
				Tac.rect(front, 0.2, 0.7, 0.6, 0.3, Tac.tone(bottom, 1.18), 3, { round = 0.4, alpha = 0.55 })
				if back then Tac.rect(back, 0.2, 0.15, 0.6, 0.4, Tac.tone(bottom, 0.9), 3, { stroke = Tac.tone(bottom, 0.6) }) end
			end
			if level >= 2 then
				local shin = paint(lowerLeg, "Front", bottom, military)
				if shin and not look.kneePads then Tac.rect(shin, 0.1, 0.12, 0.8, 0.04, Tac.tone(bottom, 0.7), 3) end
				paint(lowerLeg, side, bottom, military)
			end
		end
	end
	...
	--  Equipo
	local gc = look.gearColor
	local vest = pieces.Chaleco
	if vest then
		local front = paint(vest, "Front", gc, false, true)
		if front then
			for row = 0, 3 do		-- cintas MOLLE
				Tac.rect(front, 0.06, 0.46 + row * 0.12, 0.88, 0.045, Tac.tone(gc, 0.62), 3, { alpha = 0.15 })
			end
			local plate = Tac.rect(front, 0.28, 0.1, 0.44, 0.22, Tac.tone(gc, 0.88), 4, { round = 0.15, stroke = Tac.tone(gc, 0.6) })
			Tac.text(plate, 0.08, 0.1, 0.84, 0.8, Tac.tapeName(botName), Tac.tone(gc, 0.4), 5)
		end
		local back = paint(vest, "Back", gc, false, true)
		if back then
			for row = 0, 3 do
				Tac.rect(back, 0.06, 0.3 + row * 0.14, 0.88, 0.05, Tac.tone(gc, 0.62), 3, { alpha = 0.15 })
			end
			Tac.rect(back, 0.38, 0.03, 0.24, 0.12, Tac.tone(gc, 0.7), 4, { round = 0.3 })		-- asa
		end
		if level >= 2 then
			for _, faceName in ipairs({ "Left", "Right" }) do
				local sideFace = paint(vest, faceName, gc, false)
				if sideFace then
					Tac.rect(sideFace, 0, 0.35, 1, 0.1, Tac.tone(gc, 0.65), 3)
					Tac.rect(sideFace, 0, 0.6, 1, 0.1, Tac.tone(gc, 0.65), 3)
				end
			end
		end
	end
	for i = 1, 3 do
		local pouch = pieces["Bolsillo" .. i]
		local front = pouch and paint(pouch, "Front", pouch.Color, false)
		if front then
			Tac.rect(front, 0, 0, 1, 0.38, Tac.tone(pouch.Color, 0.8), 3, { stroke = Tac.tone(pouch.Color, 0.5) })
			Tac.rect(front, 0.4, 0.28, 0.2, 0.2, dark, 4, { round = 0.2 })
		end
	end
	local bag = pieces.Mochila
	if bag then
		local back = paint(bag, "Back", bag.Color, false)
		if back then
			Tac.rect(back, 0.04, 0.02, 0.92, 0.3, Tac.tone(bag.Color, 0.82), 3, { round = 0.25, stroke = Tac.tone(bag.Color, 0.55) })
			Tac.rect(back, 0.2, 0.3, 0.07, 0.68, Tac.tone(bag.Color, 0.65), 4)
			Tac.rect(back, 0.73, 0.3, 0.07, 0.68, Tac.tone(bag.Color, 0.65), 4)
			Tac.rect(back, 0.3, 0.55, 0.4, 0.34, Tac.tone(bag.Color, 0.9), 3, { round = 0.15, stroke = Tac.tone(bag.Color, 0.55) })
		end
	end
	local belt = pieces.Cinturon
	if belt then
		local front = paint(belt, "Front", belt.Color, false)
		if front and military then
			for _, x in ipairs({ 0.06, 0.74 }) do		-- portacargadores
				Tac.rect(front, x, 0.05, 0.2, 0.9, Tac.tone(gc, 0.85), 3, { stroke = Tac.tone(gc, 0.5) })
			end
		end
	end
	if level >= 2 then
		for _, side in ipairs({ "Left", "Right" }) do
			local boot = pieces[side .. "Cana"]
			local front = boot and paint(boot, "Front", boot.Color, false)
			if front then
				for row = 0, 2 do		-- cordones
					Tac.rect(front, 0.3, 0.15 + row * 0.28, 0.4, 0.08, Tac.tone(boot.Color, 0.55), 3)
				end
			end
			local pad = pieces[side .. "Rodillera"]
			local padFront = pad and paint(pad, "Front", pad.Color, false)
			if padFront then
				Tac.rect(padFront, 0.15, 0.2, 0.7, 0.6, Tac.tone(pad.Color, 0.8), 3, { round = 0.4, stroke = Tac.tone(pad.Color, 0.5) })
			end
		end
	end
end

--  Remeras / buzos / camperas de los civiles.
function Tac.paintShirt(front, look, rng)
	local top = look.top
	local accent = Color3.fromHSV(rng:NextNumber(), rng:NextNumber(0.4, 0.8), rng:NextNumber(0.6, 0.95))
	if look.hoodie then
		Tac.rect(front, 0.22, 0.58, 0.56, 0.3, Tac.tone(top, 0.88), 3, { round = 0.3, stroke = Tac.tone(top, 0.6) })		-- bolsillo canguro
		Tac.rect(front, 0.4, 0.02, 0.03, 0.3, Color3.fromRGB(235, 235, 235), 4)		-- cordones
		Tac.rect(front, 0.57, 0.02, 0.03, 0.3, Color3.fromRGB(235, 235, 235), 4)
		return
	end
	if look.jacket then
		local inner = look.innerColor or Color3.fromRGB(235, 235, 235)
		Tac.rect(front, 0.36, 0, 0.28, 1, inner, 2)
		Tac.rect(front, 0.35, 0, 0.02, 1, Tac.tone(top, 0.6), 3)
		Tac.rect(front, 0.63, 0, 0.02, 1, Tac.tone(top, 0.6), 3)
		Tac.rect(front, 0.06, 0.55, 0.22, 0.05, Tac.tone(top, 0.7), 3)		-- bolsillos
		Tac.rect(front, 0.72, 0.55, 0.22, 0.05, Tac.tone(top, 0.7), 3)
		return
	end
	Tac.rect(front, 0.34, 0, 0.32, 0.1, Tac.tone(top, 0.75), 3, { round = 0.5 })		-- cuello
	local design = rng:NextInteger(1, 5)
	if design == 1 then
		for i = 0, 2 do Tac.rect(front, 0, 0.3 + i * 0.18, 1, 0.07, accent, 3) end
	elseif design == 2 then
		local ring = Tac.rect(front, 0.32, 0.25, 0.36, 0.45, accent, 3, { round = 0.5 })
		Tac.rect(ring, 0.25, 0.25, 0.5, 0.5, top, 4, { round = 0.5 })
	elseif design == 3 then
		local words = Config.Names and Config.Names.Words
		Tac.text(front, 0.1, 0.3, 0.8, 0.3, string.upper(randomFrom(words, rng) or "GG"), accent, 4)
	elseif design == 4 then
		Tac.rect(front, 0.12, 0.2, 0.2, 0.18, Tac.tone(top, 0.85), 3, { stroke = Tac.tone(top, 0.6) })		-- bolsillito
	end
end
...
local function addBot()
	...
	task.spawn(function()
		local template, ids, styleName = buildTemplate(rng, role, name)
